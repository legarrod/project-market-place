  ...
  async function handlerDelete(id) {
    try {
      const { data } = await axios.delete(
        `http://localhost:3002/api/v1/products/delete/${id}`
      );
      data && getProducts();
    } catch (error) {
      console.error(error);
  ...
  async function handlerCreate(id) {
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:3002/api/v1/products/create`,
        dataToSave
      );
  ...
  async function handlerUdate(dataToSave) {
    try {
      const data = await axios.put(
        `http://localhost:3002/api/v1/products/update`,
        dataToSave
      );
      setshowFormCreate(false);
      setShowupdate(false);
          ...
          {productList.map((item) => (
            <div className="w-full flex flex-row justify-between" key={item.id}>
              ...
              <div className="flex flex-row items-center">
                ...
                <button
                  className="w-12 h-12 mx-5 flex justify-center items-center"
